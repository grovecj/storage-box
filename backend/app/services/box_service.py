...
async def create_box(db: AsyncSession, data: BoxCreate) -> BoxResponse:
    ...
    await log_action(db, box.id, "BOX_CREATED", {"box_code": box_code, "name": data.name})
    await db.commit()

    return _box_to_response(box, item_count=0, lat=lat, lng=lng)


async def get_box(db: AsyncSession, box_id: int) -> BoxResponse | None:
...
async def get_box_by_code(db: AsyncSession, box_code: str) -> BoxResponse | None:
    lat_col, lng_col = _location_columns()
    result = await db.execute(
        select(
            StorageBox,
...
async def list_boxes(
    ...
    count_result = await db.execute(select(func.count(StorageBox.id)))
...
async def update_box(db: AsyncSession, box_id: int, data: BoxUpdate) -> BoxResponse | None:
    ...
    await db.flush()
    await db.refresh(box)
    await log_action(db, box.id, "BOX_UPDATED", {"changes": data.model_dump(exclude_none=True)})
    await db.commit()

    return await get_box(db, box_id)


async def delete_box(db: AsyncSession, box_id: int) -> dict | None:
    result = await db.execute(select(StorageBox).where(StorageBox.id == box_id))
    box = result.scalar_one_or_none()
    if not box:
        return None
